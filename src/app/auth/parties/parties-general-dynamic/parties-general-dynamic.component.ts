...
export class PartiesGeneralDynamicComponent implements OnInit, OnChanges {
  ...
  getAvailableRoles(): any[] {
    ...
    if (this.isInternalActor(this.selectedActor)) {
      ...
      availableRoles = this.rolesList.filter(
        (role) => !this.isInternalActor(role.code)
      );
    }

    // Filtrar roles que ya están seleccionados
    return availableRoles.filter((role) => !existingRoles.includes(role.code));
  }
  ...
  shouldShowPersonFields(): boolean {
    const legalPersonality = this.generalForm.get(
      'identifier.legalPersonality'
    )?.value;
    return (
  ...
  private getValidationErrors(): string[] {
    ...
    if (this.shouldShowField('identifier')) {
      if (
        this.shouldShowField('legalPersonality') &&
      ...
      if (isPersonaFisica) {
        ...
        if (
          this.shouldShowField('patronymicName') &&
          !this.patronymicName.value?.trim()
        ) {
        ...
        if (
          this.shouldShowField('matronymicName') &&
          !this.matronymicName.value?.trim()
        ) {
